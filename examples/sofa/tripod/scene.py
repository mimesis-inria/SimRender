...
class Simulation(Sofa.Core.Controller):
    ...
    def __init__(self, root: Sofa.Core.Node, *args, **kwargs):
        ...
        scene.Settings.mouseButton.stiffness = 100

        tripod = create_tripod()
        scene.Modelling.addChild(tripod)
        scene.Modelling.addObject(Controller(name='TripodController', root=root, step_size=0.02, manual=False,
                                             actuators=[tripod.ActuatedArm0, tripod.ActuatedArm1, tripod.ActuatedArm2]))
        scene.Simulation.addChild(tripod)


def create_tripod(name='Tripod', radius=60, num_motors=3, angle_shift=180.0):
